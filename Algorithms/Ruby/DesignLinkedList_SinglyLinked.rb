...
=end
    def add_at_head(val)
      ...
      @head.next = Node.new(old_first_node, val)
      @size += 1
    end
...
=end
    def add_at_tail(val)
      old_last_node = get_node(@size-1)
      old_last_node.next = Node.new(@tail, val)
      @size += 1
    end
...
=end
    def add_at_index(index, val)
      ...
      before_index.next = Node.new(old_node, val)
      @size += 1
    end
...
class Node
  ...
  def initialize(next_node=nil, val=nil)
    @next, @val = next_node, val
  end
